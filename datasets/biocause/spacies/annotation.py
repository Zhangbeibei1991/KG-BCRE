SEP = "////"
DEBUG = False
def annotation_gold_entity(doc, keep_ent_tokens):
        ...
        for ann_type, ann_dict in ann_type_dict.items():
            ...
            for id_, mention in ann_dict.items():
                ...
                if matched_mention is None:
                    ...
                    token_start, token_end = get_token_indexes_from_char_span(
                    ...
                    if (token_start_char != mention_start):
                        split_token_from_char_index(doc, token_start, mention_start)
                    ...
                    if (token_end_char != mention_end):
                        split_token_from_char_index(doc, token_index, mention_end)
                    ...
                    try:
                        ...
                        print(mention.text, mention.start, mention.end, mention.type_)
                ...
                if matched_mention is not None:
                    ...
                    for token in matched_mention:
                        token._.set("span", str(mention_start + doc._.start_char) + "-" + str(
                            mention_end + doc._.start_char))
                        if ann_type == "entities":
                            ...
                            if token._.entity_type is not None:
                                ...
                                token._.set("entity_type", mention.type_)
                                token._.set("entity_id", id_)

                            if keep_ent_tokens == False:
                                spans_to_merge.append(matched_mention)

                        elif ann_type == "triggers":
                            token._.set("is_trigger", True)
                            ...
                            if token._.trigger_type is not None:
                                ...
                                token._.set("trigger_type", mention.type_)
                                token._.set("trigger_id", id_)
...
def split_token_from_char_index(doc, token_index, char_index):
...
def get_token_indexes_from_char_span(doc, char_start, char_end):
